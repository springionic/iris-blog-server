...
import (
	...
	middleware2 "iris-blog-server/server/middleware"
...
func Start() {
	...
	app := iris.New()
	app.Use(middleware2.LoggerHandler, middleware2.ErrorHandler)
	regisRouter(app)
	...
	// 终止所有日志记录器
	for i := 0; i < len(util.LoggerWriterList); i++ {
		util.LoggerWriterList[i].Stop()
	}
